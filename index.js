...
var Commands = function (hostline, user, group) {
  ...
  var file = function (user, group, file, file_link) {
    return '-rw-r--r--   1 ' + utils.formatSpaces(user, 5) + ' ' + utils.formatSpaces(group, 5) + ' 147K Mar 29  09:56 ' + '<a class="softlink-target" href=' + file_link + '>' + file + '</a>';
  }
  var softLink = function (user, group, source, target) {
    return 'lrwxrwxrwx   1 ' + utils.formatSpaces(user, 5) + ' ' + utils.formatSpaces(group, 5) + ' 4.0K Mar 29  22:09 ' + utils.spanText('softlink', source) + ' -> ' + '<a class="softlink-target" href=' + target + '>' + target + '</a>';
  }
  var ddCopyOutput = function (device, size_gb, block_size, duration) {
    ...
    bytes_copied <<= 10;
    return records + '+0 records in\n' + records + '+0 records out\n' + bytes_copied + '000 bytes (' + size_gb + ' GB) copied, ' + true_duration.toFixed(5) + ' s, ' + throughput.toFixed(1) + ' GB/s';
  }
  return {
    'poweroff': {
      ...
      hesitation: 200,
      duration: 0,
      output: "\nBroadcast message from " + hostline + "\n\t\t(/dev/pts/0) at " + utils.padNumber(new Date()
        .getHours(), 2) + ":" + utils.padNumber(new Date()
...
var Scroller = function (target) {
  ...
  updateCursorState = function (text) {
    var oldText = document.getElementById(target)
      .innerHTML || "";
    if (!text) {
      oldText += " ";
    }
